...
# ==== Set API Key ====
os.environ["OPENAI_API_KEY"] = "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"
...
vectorstore = FAISS.load_local(
    # "_faiss_10turtle_index_",
    "faiss_10turtle_openai_index",
    embedding_model,
...
def generate_followup_questions(question, answer):
    ...
    Generate 3 short, follow-up questions (max 2 lines each) based on the above Q&A.
    """
    llm = ChatOpenAI(model="gpt-4o", temperature=0.7)
...
def convert_answer_to_html(text):
    ...
    for para in paragraphs:
        ...
        if all(bullet_pattern.match(line.strip()) for line in lines if line.strip()):
            # Handle bullet list
            bullet_items = []
            for line in lines:
                ...
                if match:
                    ...
                    item_text = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', item_text)  # **bold**
                    item_text = re.sub(r'(\$\d+)', r'<strong>\1</strong>', item_text)  # $price
                    bullet_items.append(f"<li>{item_text}</li>")
            bullet_html = (
                "<ul style='margin-top: 0; margin-bottom: 11px; font-size: 14px; "
                "font-family: \"Euclid Circular A\", sans-serif;'>\n" +
                "\n".join(bullet_items) +
                "\n</ul>"
            )
            html_paragraphs.append(bullet_html)
        else:
            # Handle regular paragraph or heading-like line
            para = re.sub(r'\*\*(.*?)\*\*', r'<strong>\1</strong>', para)
            para = re.sub(r'(\$\d+)', r'<strong>\1</strong>', para)

            html_paragraphs.append(
                f'<p style="margin: 0 0 11px; font-family: \'Euclid Circular A\', sans-serif; font-size: 14px;">'
                f'{para.strip()}</p>'
            )
...
@app.get("/")
def test():
    return {"message": "OpenChatbot API is running!"}
